...
import dev.mr2.dpc.ui.RadioButtonItem
...
fun PasswordScreen(vm: MyViewModel,onNavigateUp: () -> Unit, onNavigate: (Any) -> Unit) {
    ...
    MyScaffold(R.string.password_and_keyguard, onNavigateUp, 0.dp) {
        ...
        if (privilege.device) {
            ...
            if (SP.displayDangerousFeatures) FunctionItem(R.string.max_pwd_fail, icon = R.drawable.no_encryption_fill0) { dialog = 4 }
        }
        if (VERSION.SDK_INT >= 26) {
...
fun ResetPasswordScreen(resetPassword: (String, String, Int) -> Boolean, onNavigateUp: () -> Unit) {
    ...
    var hidePassword by rememberSaveable { mutableStateOf(true) }
    var hideConfirmPassword by rememberSaveable { mutableStateOf(true) }
